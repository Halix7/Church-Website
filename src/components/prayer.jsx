...
const prayer = () => {
  ...
  function prayerProceed(e){
    e.preventDefault();
    const {name,email,address,message}=e.target;
    if(name.value.length <= 3 || email.value.length <= 0 || address.value.length <= 0 || message.value.length <=0 ){
      return  alert("Fill It Carefully!! ")
    }
    else{
  ...
  return (
    <div className='flex justify-center items-center flex-col relative'>
      ...
      <form className='flex justify-center items-center flex-col gap-3 mx-1' onSubmit={prayerProceed}>
        <Input type="text" name="name"  placeholder='Name* atleast 3 characters' className='w-[100%]' />
